...
start = time.time()
hours = 1
minutes = 40 # number of time slices
masterKeyIdx = 0
masterKeyList = {}
keyList = []
matrixes = []
for i in range(0, hours):
    for j in range(0, minutes):
        matrix, keys = buildGraphFromFile('day-2_hour-'+str(i+9)+'_minute-' + str(j) +'.txt')
        keyList.append(keys)
...
tensor = np.zeros( (len(masterKeyList), len(masterKeyList), minutes*hours), dtype=np.int8)
tIdx = 0
